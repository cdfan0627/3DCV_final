...
def training(dataset, opt, pipe, testing_iterations, saving_iterations):
    ...
    for iteration in range(1, opt.iterations + 1):
        ...
        if iteration <= opt.warm_up2 + 1000:
            Temp = 1.
        else:
            Temp = 0.1
        ...
        if iteration < opt.warm_up2:
            ...
            # min_scales, _ = torch.min(torch.abs(gaussians.get_scaling.detach() + d_scaling), dim=1)
            # max_scales, _ = torch.max(torch.abs(gaussians.get_scaling.detach() + d_scaling), dim=1)
            # min_c_scales, _ = torch.min(gaussians.get_scaling.detach(), dim=1)
            # print("short :", min_sclaes.min())
            # print("min_c_scales: ", min_c_scales.min())
            reg_loss = torch.mean(torch.norm(deform_deltanormal, p=2, dim=1))
            #reg_loss = torch.mean(torch.norm(deform_deltanormal, p=2, dim=1))
            pred_normal = render_pkg_re["normal"].to('cuda')
            gt_normal = viewpoint_cam.kwargs['normal_map'].permute(2, 0, 1).to('cuda')
            normal_loss = torch.abs((pred_normal - gt_normal)).mean()
        ...
        with torch.no_grad():
            ...
            if iteration % 10 == 0:
                progress_bar.set_postfix({"Loss": f"{Ll1:.{4}f}", "normal loss": f"{normal_loss:.{4}f}", "reg loss": f"{reg_loss:.{4}f}"})
            ...
            if config['cameras'] and len(config['cameras']) > 0:
                ...
                for idx, viewpoint in enumerate(config['cameras']):
                    ...
                    if iteration < opt.warm_up:
                        ...
                        rotation_matrix = rotation_matrix_from_vectors(normal, deform_normal)
                        deform_deltanormal = torch.matmul(rotation_matrix, scene.gaussians.get_delta_normal.unsqueeze(-1)).squeeze(-1)
                        new_normal = safe_normalize(deform_normal + deform_deltanormal)
                        reflvec = safe_normalize(reflect(deform_wo, new_normal))
                        spat = torch.cat([scene.gaussians.get_xyz.detach() + d_xyz.detach(), scene.gaussians.get_roughness * torch.abs(scene.gaussians.get_scaling.detach() + d_scaling.detach()), scene.gaussians.get_rotation.detach() + d_rotation.detach()], dim=-1)
                        spec_color = specdecoder.step(spat, reflvec)
                    image = torch.clamp(
...
if __name__ == "__main__":
    ...
    parser.add_argument("--save_iterations", nargs="+", type=int, default=list(range(1000, 40001, 1000)))
