...
class GaussianModel:
    def __init__(self, sh_degree: int):
        ...
        self.roughness_activation = torch.nn.Softplus()
    ...
    def create_from_pcd(self, pcd: BasicPointCloud, spatial_lr_scale: float):
        ...
        colors_copy = pcd.colors.copy()
        fused_point_cloud = torch.tensor(np.asarray(pcd.points)).float().cuda()
        fused_color = RGB2SH(torch.tensor(np.asarray(pcd.colors)).float().cuda())
        features = torch.zeros((fused_color.shape[0], 3, (self.max_sh_degree + 1) ** 2)).float().cuda()
        features[:, :3, 0] = fused_color
        features[:, 3:, 1:] = 0.0
        fused_color_d = torch.tensor(np.asarray(colors_copy)).float().cuda()
        features_d = torch.zeros((fused_color_d.shape[0], 3)).float().cuda()
        features_d[:, :3 ] = fused_color_d
        features_d[:, 3: ] = 0.0
        ...
        self._diffuse_color = nn.Parameter(features_d[:,:3].contiguous().requires_grad_(True))
        specular_len = 3 
    ...
    def training_setup(self, training_args):
        ...
        self._diffuse_color.requires_grad_(requires_grad=False)
        l.extend([
            {'params': [self._features_dc], 'lr': training_args.feature_lr, "name": "f_dc"},
            {'params': [self._features_rest], 'lr': training_args.feature_lr / 20.0, "name": "f_rest"},
            {'params': [self._diffuse_color], 'lr': training_args.diffuse_lr, "name": "diffuse_color"},
    ...
    def construct_list_of_attributes(self):
        ...
        for i in range(self._diffuse_color.shape[1]):
    ...
    def save_ply(self, path):
        ...
        attributes = np.concatenate((xyz, normals, f_dc, f_rest, diffuse_color, opacities, scale, rotation, roughness, specular), axis=1)
    ...
    def load_ply(self, path, og_number_points=-1):
        ...
        diffuse_color = np.zeros((xyz.shape[0], 3))
        diffuse_color[:, 0] = np.asarray(plydata.elements[0]["diffuse_color_0"])
        diffuse_color[:, 1] = np.asarray(plydata.elements[0]["diffuse_color_1"])
        diffuse_color[:, 2] = np.asarray(plydata.elements[0]["diffuse_color_2"])
